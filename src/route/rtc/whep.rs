...
pub fn route() -> Router<AppState> {
    Router::new().route("/stream/whep/:json_base64/", post(whep))
}
...
async fn whep(Path(params): Path<HashMap<String, String>>) -> Result<Response> {
                ...
                if client.check_token(json.user_token.clone()) {
                    ...
                    let (_peer, answer, _session) = forwarder
                        .subscribe(
                            ...
                            id,
                            offer,
                            Box::new(move |_candidate: Option<RTCIceCandidate>| Box::pin(async {})),
                        )
                        .await?;
                    drop(forwarder);
